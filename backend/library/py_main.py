"""
BESTO Design - 백엔드 전용 계산 진입점.
py_base, py_base_sub, js 미사용. get_beam_info, get_neighbor_h_beams, calculate_design_strength 제공.
"""
import json
import copy
import math

# BESTO Design Library import (backend library 패키지 내)
from library.py_library import (
    HBeamData,
...
# 설계 기본값 import
from library.py_config import (
    get_default_design_inputs,
)


def main():
    """백엔드 전용 no-op (Pyscript 엔트리 불필요)."""
    pass
...
def get_neighbor_h_beams(selected_member):
    """
    선택된 H형강 부재를 기준으로 H(높이) 값으로 위 아래 5개씩 선택한 후,
    그 중에서 Zx 값이 작은 것 2개, 큰 것 2개를 선택하여 총 5개 부재 리스트 반환
    
    Args:
        selected_member: 선택된 H형강 이름 (예: "H-600X200X11X17")
    
    Returns:
        JSON string: H형강 이름 리스트 (Zx 값 기준으로 정렬)
    """
    if isinstance(selected_member, str):
        # JSON string인 경우 파싱 시도
        try:
            data = json.loads(selected_member)
            selected_member = data.get('selectedMember', selected_member)
        except:
            pass
    
    if selected_member not in HBeamData:
        return json.dumps({'error': f'H-section {selected_member} not found'}, ensure_ascii=False)
    
    # 1. H형강 데이터를 H(높이) 값 기준으로 정렬
    sorted_by_h = sorted(
        HBeamData.items(),
        key=lambda x: (x[1]['H'], x[1]['B'])  # 높이, 너비 순으로 정렬
    )
    
    # 2. 선택된 부재의 인덱스 찾기
    selected_index = None
    for i, (name, data) in enumerate(sorted_by_h):
        if name == selected_member:
            selected_index = i
            break
    
    if selected_index is None:
        return json.dumps({'error': f'Selected member {selected_member} not found in sorted list'}, ensure_ascii=False)
    
    # 3. H 기준으로 위 5개, 아래 5개 추출 (총 11개)
    start_index = max(0, selected_index - 5)
    end_index = min(len(sorted_by_h), selected_index + 6)  # +6은 inclusive
    
    # H 기준으로 선택된 부재 주변 11개 추출
    h_neighbor_beams = sorted_by_h[start_index:end_index]
    
    # 4. 추출된 11개를 Zx 값 기준으로 정렬
    sorted_by_zx = sorted(
        h_neighbor_beams,
        key=lambda x: x[1]['Zx']  # Zx 값으로 정렬
    )
    
    # 5. Zx가 작은 것 2개, 큰 것 2개 선택
    zx_smallest_2 = [name for name, _ in sorted_by_zx[:2]]  # Zx가 작은 것 2개
    zx_largest_2 = [name for name, _ in sorted_by_zx[-2:]]  # Zx가 큰 것 2개
    
    # 6. 선택된 부재 + 작은 2개 + 큰 2개 = 총 5개 (중복 제거)
    neighbor_beams = list(set([selected_member] + zx_smallest_2 + zx_largest_2))
    
    # 7. 최종 결과를 Zx 값 기준으로 정렬하여 반환
    final_sorted = sorted(
        neighbor_beams,
        key=lambda name: HBeamData[name]['Zx']
    )
    
    return json.dumps(final_sorted, ensure_ascii=False)
...
def _run_design_calculation(config):
    ...
    try:
        H_height = float(config.get('h_height', h_data.get('H')))
        H_width = float(config.get('h_width', h_data.get('B')))
        H_web_thickness = float(config.get('h_t1', h_data.get('t1')))
        H_flange_thickness = float(config.get('h_t2', h_data.get('t2')))
    except (TypeError, ValueError):
        raise ValueError(f'H-section {h_section_name} info incomplete. Provide dimensions or a valid section name.')
    ...
    try:
        U_height = float(config.get('u_height', u_match.get('U_height')))
        U_width = float(config.get('u_width', u_match.get('U_width')))
        U_thickness = float(config.get('u_thickness', u_match.get('U_thickness')))
    except (TypeError, ValueError):
        raise ValueError(f'U-section matching {h_section_name} info incomplete. Provide dimensions or a valid section name.')
    ...
    result = {
        ...
        'sectionInfo': {
            ...
            'uModulusX1': round(U_section.sectionModulusX1, 2),
            'hNeutralAxis': round(H_section.centerY, 2),
            'uNeutralAxis': round(U_section.centerY, 2),
            'compositeArea': round(composite_section.area, 2),
            'compositeInertia': round(composite_section.inertiaX, 0),
            'compositeModulusX1': round(composite_section.sectionModulusX1, 2),
            'compositeNeutralAxis': round(composite_section.centerY, 2),
            'effectiveWidth': round(effective_width, 2),
            'uPlasticNeutralAxis': round(U_section.plasticNeutralAxis, 2),
            'hPlasticNeutralAxis': round(H_section.plasticNeutralAxis, 2),
            'compositePlasticNeutralAxis': round(composite_section.plasticNeutralAxis, 2),
        },
        'materialInfo': {
            'steelFyH': steel_Fy_H,
            'steelFyU': steel_Fy_U,
        ...
        'constructionStage': {
            'U_positive': {
                ...
                'yieldStrength': round(U_nominal_M_positive.elasticMomentStrength / 1e6, 2),
                'flangeWT': {
                    'wtRatio': round(U_nominal_M_positive.flange_WTRatio.wtRatio, 2),
                    'lambda_p': round(U_nominal_M_positive.flange_WTRatio.lambda_p, 2),
                    'lambda_r': round(U_nominal_M_positive.flange_WTRatio.lambda_r, 2),
                    'check': U_nominal_M_positive.flange_WTRatio.check(),
                },
                'check': 'OK' if U_moment_check_positive else 'NG',
                'ratio': round(U_required_M_positive / U_design_M_positive, 2) if U_design_M_positive > 0 else 0,
            },
            'U_negative': {
                ...
                'yieldStrength': round(U_nominal_M_negative.elasticMomentStrength / 1e6, 2),
                'flangeWT': {
                    'wtRatio': round(U_nominal_M_negative.flange_WTRatio.wtRatio, 2),
                    'lambda_p': round(U_nominal_M_negative.flange_WTRatio.lambda_p, 2),
                    'lambda_r': round(U_nominal_M_negative.flange_WTRatio.lambda_r, 2),
                    'check': U_nominal_M_negative.flange_WTRatio.check(),
                },
                'check': 'OK' if U_moment_check_negative else 'NG',
                'ratio': round(U_required_M_negative / U_design_M_negative, 2) if U_design_M_negative > 0 else 0,
            },
            'U_shear': {
                ...
                'yieldStrength': round(U_nominal_V.shearStrength / 1e3, 2),
                'shearCoefficient': round(U_nominal_V.shearCoefficient, 2),
                'slendernessRatio': round(U_nominal_V.slendernessRatio, 1),
                'yieldLimit': round(U_nominal_V.yieldLimit, 1),
                'bucklingLimit': round(U_nominal_V.bucklingLimit, 1),
                'check': 'OK' if U_shear_check else 'NG',
                'ratio': round(U_required_V / U_design_V, 2) if U_design_V > 0 else 0,
            },
            'H_negative': {
                ...
                'yieldStrength': round(H_nominal_M.elasticMomentStrength / 1e6, 1),
                'lateralBucklingStrength': round(H_nominal_M.lateralBucklingStrength / 1e6, 1),
                'Lb': round(H_nominal_M.unbracedLength / 1e3, 2),
                'Lp': round(H_nominal_M.plasticUnbracedLength / 1e3, 2),
                'Lr': round(H_nominal_M.elasticUnbracedLength / 1e3, 2),
                'ratio': round(H_required_M_negative / H_design_M_negative, 2) if H_design_M_negative > 0 else 0,
            ...
            'H_shear': {
                ...
                'yieldStrength': round(H_nominal_V.shearStrength / 1e3, 1),
                'shearCoefficient': round(H_nominal_V.shearCoefficient, 1),
                'slendernessRatio': round(H_nominal_V.slendernessRatio, 1),
                'yieldLimit': round(H_nominal_V.yieldLimit, 1),
                'bucklingLimit': round(H_nominal_V.bucklingLimit, 1),
                'ratio': round(H_required_V / H_design_V, 1) if H_design_V > 0 else 0,
        ...
        'compositeStage': {
            ...
            'U_positive': {
                ...
                'webWT': {
                    'wtRatio': round(Comp_web_WT.wtRatio, 2),
                    'lambda_p': round(Comp_web_WT.lambda_p, 2),
                    'lambda_r': round(Comp_web_WT.lambda_r, 2),
                    'check': Comp_web_WT.check(),
                },
                'compositeRatio': round(composite_ratio * 100, 1),
                'check': 'OK' if Comp_moment_check_positive else 'NG',
                'ratio': round(Comp_required_M_positive / Comp_design_M_positive, 2) if Comp_design_M_positive > 0 else 0,
            },
            'U_negative': {
                ...
                'bottomFlangeWT': {
                    'wtRatio': round(Comp_bottom_flange_WT.wtRatio, 2),
                    'lambda_p': round(Comp_bottom_flange_WT.lambda_p, 2),
                    'lambda_r': round(Comp_bottom_flange_WT.lambda_r, 2),
                    'check': Comp_bottom_flange_WT.check(),
                },
                'check': 'OK' if Comp_moment_check_negative_U else 'NG',
                'ratio': round(Comp_required_M_negative_U / Comp_design_M_negative_U, 2) if Comp_design_M_negative_U > 0 else 0,
            },
            'H_negative': {
                ...
                'webWT': {
                    'wtRatio': round(H_web_WT.wtRatio, 2),
                    'lambda_p': round(H_web_WT.lambda_p, 2),
                    'lambda_r': round(H_web_WT.lambda_r, 2),
                    'check': H_web_WT.check(),
                },
                'bottomFlangeWT': {
                    'wtRatio': round(H_bottom_flange_WT.wtRatio, 2),
                    'lambda_p': round(H_bottom_flange_WT.lambda_p, 2),
                    'lambda_r': round(H_bottom_flange_WT.lambda_r, 2),
                    'check': H_bottom_flange_WT.check(),
                },
                'check': 'OK' if Comp_moment_check_negative_H else 'NG',
                'ratio': round(Comp_required_M_negative_H / Comp_design_M_negative_H, 2) if Comp_design_M_negative_H > 0 else 0,
            },
            'shear': {
                'requiredStrength': round(Comp_required_V / 1e3, 2),
                'designStrength': round(U_design_V / 1e3, 2),
                'yieldStrength': round(U_nominal_V.shearStrength / 1e3, 1),
                'shearCoefficient': round(U_nominal_V.shearCoefficient, 2),
                'slendernessRatio': round(U_nominal_V.slendernessRatio, 1),
                'yieldLimit': round(U_nominal_V.yieldLimit, 1),
                'bucklingLimit': round(U_nominal_V.bucklingLimit, 1),
                'check': 'OK' if Comp_shear_check else 'NG',
                'ratio': round(Comp_required_V / U_design_V, 2) if U_design_V > 0 else 0,
            },
            'deflection': {
                'compositeRatio': round(composite_ratio * 100, 1),
        ...
        'checkResult': {
            'U_moment_check_positive': U_moment_check_positive,
            'U_moment_check_negative': U_moment_check_negative,
            'U_shear_check': U_shear_check,
            'H_moment_check_negative': H_moment_check_negative,
            'H_shear_check': H_shear_check,
            'deflection_check_construction': deflection_check_construction,
            'Comp_moment_check_positive': Comp_moment_check_positive,
            'Comp_moment_check_negative_U': Comp_moment_check_negative_U,
            'Comp_moment_check_negative_H': Comp_moment_check_negative_H,
            'Comp_shear_check': Comp_shear_check,
            'deflection_check_live': deflection_check_live,
            'deflection_check_dead_live': deflection_check_dead_live,
        },
        'parsedInputs': {
            'hSection': {
                'height': H_height,
